...
export const quote      = (children = [], author = null) =>
  ({ type: 'Quote', author, children });

export const list       = (ordered = false, children = []) =>
  ({ type: 'List', ordered, children });
export const listItem   = (children = []) => ({ type: 'ListItem', children });

export const lineBreak  = () => ({ type: 'LineBreak' });

export const spoiler    = (children = [], label = null) =>
  ({ type: 'Spoiler', label, children });

export const heading    = (value = '', depth = 1) =>
  ({ type: 'Heading', value, depth: Math.min(6, Math.max(1, depth)) });
